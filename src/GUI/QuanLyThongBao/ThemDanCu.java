 ...
 */
package GUI.QuanLyThongBao;

import GUI.QuanLyThongBao.ChiTietThongBao;
import Model.ThongBao;
import Model.DanCu;
import static Services.ThongBaoService.SVFromFile;
import static Services.ThongBaoService.getListSVById;
import static Services.ThongBaoService.writeFile;
 ...
 * @author Đặng Huy
 */
public class ThemDanCu extends javax.swing.JFrame {
    ...
    List<DanCu> listSV;
    List<String> listMSV;
    List<ThongBao> listLichThi;
    ThongBao lichthi;
    static DefaultTableModel tbSinhVien;
    static String fileName = "src/FileData/ThongBao.txt";
    static String fileSV = "src/FileData/DanCu.txt";
    private ChiTietThongBao tsScreen;
    private DanCu searchSV;

    public ThemDanCu(List listMaSV, ThongBao lichthiFromParent, List<ThongBao> list, ChiTietThongBao tsScreen) {
        initComponents();
        ...
        tbSinhVien = (DefaultTableModel) tableSinhVien.getModel();
        try {
            listSV = SVFromFile(fileSV);
        } catch (Exception ex) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Thêm dân cư");

        tableSinhVien.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        tableSinhVien.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã cư dân", "Họ tên", "Ngày sinh", "Giới tính", "Quê quán"
            }
        ) {
        ...
        jLabel2.setText("Tìm dân cư theo mã");
        ...
        btnThemThiSinh1.setText("Thêm cư dân");
    ...
    private void btnThemThiSinh1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemThiSinh1ActionPerformed
        int[] selectedRows = tableSinhVien.getSelectedRows();
        if (selectedRows.length <= 0) {
            JOptionPane.showMessageDialog(this, "Hãy chọn ít nhất một dòng để thêm!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
        } else {
            int confirm = JOptionPane.showConfirmDialog(this, "Thêm những dân cư đã chọn", "Xác nhận thêm", JOptionPane.INFORMATION_MESSAGE);
            if (searchSV == null) {
                if (confirm == JOptionPane.YES_OPTION) {
                    for (int row : selectedRows) {
                        for (ThongBao lich : listLichThi) {
    ...
    private void txtMaSVKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtMaSVKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (txtMaSV.getText().trim().equals("")) {
                JOptionPane.showMessageDialog(this, "Nhập mã dân cư để tìm kiếm!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            } else {
                if (searchSV == null) {
                    JOptionPane.showMessageDialog(this, "Không có dân cư này hoặc đã có trong thông báo!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                } else {
    ...
    private void showTableData() {
        tbSinhVien.setRowCount(0);
        List<DanCu> listShow = new ArrayList<>();
        listShow = listSV;
        List<DanCu> listExists = new ArrayList<>();
        for (int i = 0; i < listMSV.size(); i++) {
            listExists.add(getListSVById(listSV, listMSV.get(i)));
        }
        listShow.removeAll(listExists);
        for (DanCu sv : listShow) {
            tbSinhVien.addRow(new Object[]{sv.getMaSV(), sv.getTenSV(), sv.getKhoa(), sv.getNgaySinh(), sv.getGioiTinh()});
        }
    }

    private DanCu searchSVbYId(String MaSV) {
        DanCu sv = new DanCu();
        for (int i = 0; i < listMSV.size(); i++) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ThemDanCu.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
